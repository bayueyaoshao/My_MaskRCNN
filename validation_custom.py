        ...
        for image, targets in tqdm(val_dataset_loader, desc="validation..."):
            ...
            for i, score in enumerate(scores):
                if score > 0.8:
                    ...
                    # 绘制边界框
                    cv2.rectangle(img, (int(xmin.item()), int(ymin.item())), (int(xmax.item()), int(ymax.item())), color, thickness)
                    # 保存图像
                    

                    # # cv2.rectangle(img, (int(xmin.item()), int(ymin.item())), (int(xmax.item()), int(ymax.item())), color, thickness)
                    # cv2.rectangle(img, (200, 200), (400, 400), color, thickness)
                    # count = count+1
                    # img1 = np.transpose(img, (1, 2, 0))
                    # cv2.imwrite(f'image_with_boxes_{count}.jpg', img1)
            cv2.imwrite(f'{result_path}/image_with_boxes_{count}.jpg', img)
            count += 1
...
if __name__ == "__main__":
    ...
    # 数据集的根目录
    parser.add_argument('--data-path', default=r'D:\Work\Git-Code\My_MaskRCNN\data\CustomDataset', help='dataset root')
